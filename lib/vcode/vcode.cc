...
VReg VCode::append( const MachInstKind kind, const VReg result, const std::initializer_list< Operand > ops, const i64 imm,
                    const MemRef mem )
{
...
void VCode::append_void_epi( const MachInstKind kind, const std::initializer_list< Operand > ops, const i64 imm,
                         const MemRef mem )
{
    assert( ops.size( ) <= std::numeric_limits< u16 >::max( ) );
