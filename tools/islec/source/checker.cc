...
void checker::check( )
{
    ...
    for ( const auto &n : m_ast )
        if ( const auto *r = std::get_if< rule_node >( &n ) ) check_rule( *r );
}
...
void checker::collect_decls( )
{
    for ( const auto &no : m_ast )
    {
        std::visit( [ & ]< typename Ty >( const Ty &n )
        {
            ...
            if constexpr ( std::is_same_v< T, type_decl_node > )
            {
                if ( !m_types.insert( n.name ).second ) throw std::runtime_error( std::format( "duplicate type decl '{}'", n.name ) );
            }
            else if constexpr ( std::is_same_v< T, extern_decl_node > )
            {
...
void checker::check_rule( const rule_node &r )
{
    check_expr_patt( r.pattern );
    check_expr_body( r.body );
}

void checker::check_expr_patt( const expr_node &e )
{
...
void checker::check_expr_body( const expr_node &e )
{
    if (e.args.empty( ) ) return;

    if (!m_constructors_sym_table.contains( e.head ) )
